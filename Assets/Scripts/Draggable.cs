...
public class Draggable : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public float yPos2BeExtended;//확장된 카드가 세워질 고정 y 좌표.
    ...
    void Start()
    {
        ...
        float cardHalfY = card.transform.GetComponent<RectTransform>().rect.height * (HandingManager.Instance.cardMaxSize + 0.7f) / 1f;
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        if (HandingManager.Instance.endDraw)
        {
            ...
            {
                HandingManager.Instance.battle.card_Sound_Obj.SetActive(false);
                HandingManager.Instance.battle.card_Sound.soundClip = HandingManager.Instance.battle.card_Sounds[2];
                HandingManager.Instance.battle.card_Sound_Obj.SetActive(true);
            }
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        if (HandingManager.Instance.endDraw && mousePointFollow)
        {
            ...
            if (!isOnDropZone)
            {
                if (!HandingManager.Instance.battle.card_Sound_Obj.activeSelf)
                {
                    HandingManager.Instance.battle.card_Sound.soundClip = HandingManager.Instance.battle.card_Sounds[1];
                    HandingManager.Instance.battle.card_Sound_Obj.SetActive(true);
                }
                else
                {
    ...
    {        
        if (!isOnDropZone && card.isDraggable && !mousePointFollow && HandingManager.Instance.endDraw && !HandingManager.Instance.dragCard && !mouseOver && !avoidOverlap)
        {
            ...
            ChangeTransform(ObjectControl.RotationAngle(gameObject, 0f), HandingManager.Instance.cardMaxSize + 0.3f, new Vector3(transform.localPosition.x, yPos2BeExtended, -1f));
    ...
    //카드의 크기, 회전, 이동을 즉시 변경.
    public void ChangeTransform(float varAngle, float varScale, Vector3 varPos)
    {
