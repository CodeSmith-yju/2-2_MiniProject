...
public class HandingManager : MonoBehaviour
{
    ...
    public int drawableCount = 5;//드로우할 카드의 수.
    ...
    public bool isDrawChecked = false;
    ...
    public IEnumerator Card_Cost_Chk()
    {
        yield return new WaitForSeconds(1f);
        bool has_Card = false;
        foreach (Card hand in cardList)
        {
            if (hand.init.cost <= battle.cur_Cost)
            {
                battle.end_Btn.image.sprite = battle.btn_Sp[0];
                has_Card = true;
                break;
            }
        ...
        if (!has_Card)
        {
            ...
            {
                battle.end_Btn.image.sprite = battle.btn_Sp[1];
            }
            else
            {
                battle.end_Btn.image.sprite = battle.btn_Sp[0];
    ...
    public IEnumerator DrawDelay()
    {
        yield return new WaitForSeconds(1.5f);
        HandingManager.Instance.ReDraw();
    }

    public void PlusDraw(int cnt)
    {
        StartCoroutine(PlusDraw_Logic(cnt));
    }


    private IEnumerator PlusDraw_Logic(int cnt)
    {
        ...
        if (GameManager.turn == GameManager.Turn.PlayerTurn)
        {
            ...
            {
                cardList[i].isDraggable = false;
            }

            int card_Cnt = cardList.Count;

            for (int i = card_Cnt; i < (card_Cnt + cnt); i++)
            {
                StartCoroutine(CardDraw_SoundDelay());
                ...
                if (i >= maxCardCount)
                {
                    if(i >= 11)
                    {
                        DropCard(0.2f, cardList[i - 1].order);
                    }
                    else
                    {
                        DropCard(0.2f, cardList[i].order);
                    }
                }

                else
                {
                    StartCoroutine(DrawCardC(0.2f, cardList[i].order));

                    battle.card_Sound.soundClip = battle.card_Sounds[0];
                    battle.card_Sound_Obj.SetActive(true);
                }
    ...
    public IEnumerator CardDraw_SoundDelay()
    {
        yield return new WaitForSeconds(0.18f);
        battle.card_Sound_Obj.SetActive(false);
    }
    //드로우하는 과정. 
    ...
    {
        StartCoroutine(ObjectControl.RotationToC(0f, new Vector3(0f, 0f, -180f), cardList[index].gameObject));
        SetCurveRate(index);
        ...
        StartCoroutine(cardList[index].SetActiveOfTrailC(0.1f, false));
    ...
    private IEnumerator TurnEndDropAllCardC()
    {
        float time = 0.2f;
        foreach (Card card in cardList)
        {
    ...
    public void TurnEndReDraw()
    {
        ...
        if (isDrawable)
        {
            isDrawable = false;
            isDrawChecked = false;
            StartCoroutine(TurnEndDropAllCardC());
        }
    }
    public void BattleEnd()
    {
        ...
        {
            Debug.Log("전투 종료");
            GameManager.kill_Init++;
            GameManager.turn = GameManager.Turn.BattleEnd;
        }

        else if (GameManager.player_Dead_Check)
        {
            Debug.Log("게임 오버");
            GameManager.turn = GameManager.Turn.GameOver;
